...
t=[0,1,2]  # each feature can be 0/1/2 according to the code e.g. 0012v3
randomtul=random.sample(t,len(t)) # randomizing the features e.g. [2,0,1] 
tul_egyA=str(randomtul[0]) # selecting the first and second feature from the list as first feature for A and B categories
tul_egyB=str(randomtul[1])
randomtul=random.sample(t,len(t)) # randomizing the feature list again
tul_kettoA=str(randomtul[0]) # selecting the first and second feature from the list as second feature for A and B categories
tul_kettoB=str(randomtul[1])
randomtul=random.sample(t,len(t)) # randomizing the feature list again
tul_haromA=str(randomtul[0]) # selecting the first and second feature from the list as third feature for A and B categories
tul_haromB=str(randomtul[1])
randomtul=random.sample(t,len(t)) # randomizing the feature list again
tul_negyA=str(randomtul[0]) # selecting the first and second feature from the list as forth feature for A and B categories
tul_negyB=str(randomtul[1])
...
instruction_text.setText("In the followings creatures will appear one by one. \n There are two types of them: A and L. \n Your task is to figure out the type of the creature and press the corresponding button (A or L)\n After each choice you will get a feedback whether your decision was right or wrong. \n After the learning phase a test phase follows where you won't recieve feedback and your reaction time and accuracy will be measured. \n If you are ready to start press Enter!")
instruction_text.draw()
win.flip()
event.waitKeys(keyList=['return'])


for a in range(0,3): # goes through all training exemplars 3 times
    ffp_random=random.sample(ffp,len(ffp)) # each time a new random order is used

    for dino in ffp_random: # goes through the randomized list of exemplars
        training_presented.append((dino_dict.keys()[dino_dict.values().index(dino)])) # finds the appropriate file name for the exemplar code from the dictionary
        trial_clock.reset()
        ...
        stimuli.setImage(exp_info['training stimuli directory'] + '\stim15_'+ dino + '.jpg') # sets the stimuli
        stimuli.draw()
        win.flip()
        ...
        if ((press[0] == 'a') & (dino in ffpA)) | ((press[0] == 'l') & (dino in ffpB)): # checks if the answer was correct
                feedback_text.setText(feedback[0]) # sets the feedback massage to correct
                feedback_text.draw()
                win.flip()
                core.wait(0.5)
                answers.append('1') # logs that it was correct
        else:
                feedback_text.setText(feedback[1]) # sets feedback massage to incorrect
                feedback_text.draw()
                win.flip()
                core.wait(0.5)
                answers.append('0') # logs that it was incorrect
order=[i for i in range(1,(3*len(ffp))+1)] # makes a list of the numbers from 1 to the length of the training
log = zip(order,training_presented, answers, RTs) # puts together the order number, the code, the hit and the RT for each training stimuli


#test
instruction_text.setText('The training is over. \n Your task is the same but this time you will not get any feedback.\n Your reaction time will be registered so be as fast and as accurate as possible. \n If you are ready press Enter!')
instruction_text.draw()
win.flip()
event.waitKeys(keyList=['return'])
...
# Making log files
file_name = exp_info['participant']
...
file.write('Training'+ "\n")
for order, name, correct, time in log:
    file.write("\n".join([" %s %s %s %s" % (order, name, correct, time)]) + "\n")
file.write('Test - FFP'+ "\n")
